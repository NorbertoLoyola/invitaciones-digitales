...
### Personalización instantánea en el catálogo — probada y luego sacada (2026-08-16)

Se implementó como un campo **opcional** arriba de la barra de chips ("¿Para quién es la fiesta?"), con una frase corta por categoría reflejada en vivo en cada card (`fraseParaCategoria`, switch con una frase por categoría — ej. "Bodas" → "{Nombre} se casa 💍"). Funcionaba bien, pero una vez que se sumó el overlay de personalización al abrir el template real (ver abajo), el usuario notó que pedía el nombre **dos veces** — una en el catálogo, otra en el overlay — y pidió sacarlo del catálogo: "me parece mucho pedir dos veces el nombre, lo sacaría de ahí y lo dejaría como lo implementaste ahora". Se revirtió por completo: sin campo, sin frase en las cards, "Ver ejemplo" vuelve a ser un link plano sin `?nombre=`. El overlay de cada template sigue leyendo `?nombre=` de la URL como pre-llenado por si alguna vez se comparte un link directo así, pero ya no hay ninguna fuente que lo mande — queda inerte y es inofensivo dejarlo (evita tocar los 16 templates de nuevo por algo que no rompe nada).
...
- El overlay lee `?nombre=<valor>` de la URL como **pre-llenado** del primer campo (nunca se aplica solo — hace falta tocar "Ver con mi nombre"), pero el catálogo ya no genera ese parámetro (ver sección de arriba, sacado el 2026-08-16 por pedir el nombre dos veces). Confirmado en vivo cuando existía: con `?nombre=Prellenada` en la URL, el input aparecía prellenado pero `CONFIG.nombre` seguía en el valor demo hasta tocar "Ver con mi nombre" — ese comportamiento de pre-llenado sigue intacto en cada template, solo que hoy nada lo dispara.
